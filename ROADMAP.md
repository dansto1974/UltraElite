...
- **V2b renderer convergence, mostly landed** — normal flight, external view, ship preview/library, hangar ships, cargo props and cut-scene ships now share the same `drawModelEntity` / `modelMeshForRender` path, including clipping, culling, solid/wire/detail drawing, decals, engine glows and trails. This killed most of the repeated transform bugs and made future ship/object changes much safer.

## Next up (small/medium)
- **V2b renderer polish** — finish the last tidy-up rather than doing another grand refactor: document the canonical render path, keep previews/cutscenes on it, remove any leftover duplicate ship/object drawing when found, and preserve special-case renderers only for things that genuinely are not mesh objects (planet surfaces, rings, stars, laser/particle effects, hangar architecture).
...
- **`ultra-elite-render-rules`** — hard-won renderer rules: world/object-space textures, shared camera transforms, near-plane clipping, glow ordering, ring/station portal behaviour and Old School visual boundaries.
